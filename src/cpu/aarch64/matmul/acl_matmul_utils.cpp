...
bool is_batch_ordered(const memory_desc_wrapper &mdw) {
...
status_t init_conf_matmul(acl_matmul_conf_t &amp, memory_desc_t &src_md,
    ...
    ACL_CHECK_SUPPORT(
            !is_batch_ordered(src_d), "src batch dimensions must be in order");
    ACL_CHECK_SUPPORT(
            !is_batch_ordered(wei_d), "wei batch dimensions must be in order");
    ACL_CHECK_SUPPORT(
            !is_batch_ordered(dst_d), "dst batch dimensions must be in order");
